...
_RECENCY_RE = re.compile(
    r"\b(latest|most recent|newest|recently|recent|last submitted|recently submitted|recent submission)\b",
    re.I,
)
...
- For "latest", "most recent", or "last submitted" questions: look at the dates shown in the source headers and identify the record with the most recent date. State that date and the activity clearly.
"""
...
class HybridRAGChain:
    ...
    async def query(
        ...
        # Inject recency filter for "latest"/"most recent" queries
        if _RECENCY_RE.search(query) and "submitted_at__gte" not in filters:
            thirty_days_ago = (datetime.now(timezone.utc) - timedelta(days=30)).strftime("%Y-%m-%d")
            filters["submitted_at__gte"] = thirty_days_ago
        ...
        # For recency queries: skip reranker (it ranks by semantic match, not date)
        # and sort fused results by submitted_at DESC so newest is always Source 1.
    ...
    def _format_context(chunks: list[RetrievedChunk]) -> str:
        ...
        for i, c in enumerate(chunks, 1):
            date_str = c.submitted_at[:10] if c.submitted_at else "unknown"
            parts.append(
                f"[Source {i} | type={c.source_type} | activity={c.activity_name} | date={date_str}]\n{c.chunk_text}"
            )
        return "\n\n---\n\n".join(parts)
