...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'ADD_USER': return { ...state, messages: [...state.messages, { role: 'user', content: action.content, ts: Date.now() }], input: '', isStreaming: true }
...
export function useChat() {
  ...
  const send = useCallback(async (text: string) => {
    if (!text.trim() || stateRef.current.isStreaming) return
