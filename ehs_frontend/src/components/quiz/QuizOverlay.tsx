...
export function QuizOverlay({ quiz, activity, onClose }: Props) {
  ...
  const { phase, questions, currentIdx, mcqAnswers, textAnswers, correctCount, mcqTotal, selectMcq, setText, next, submit, close } = quiz
  ...
  const scoreColor = mcqTotal > 0
    ? (correctCount / mcqTotal >= 0.8 ? '#1F7A3A' : correctCount / mcqTotal >= 0.5 ? '#B26A00' : '#C4302B')
    : '#5A6272'

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(11,18,32,0.67)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'stretch', justifyContent: 'center', animation: 'fadeIn .25s ease-out' }}>
      <div style={{ background: '#F5F2EC', width: '100%', maxWidth: 880, margin: '48px auto', borderRadius: 6, overflow: 'hidden', animation: 'slideUp .4s cubic-bezier(.2,.8,.25,1)', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(11,18,32,0.25)' }}>
        ...
        <div style={{ padding: '18px 32px', background: '#FFFFFF', borderBottom: '1px solid #E4DFD3', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {phase === 'results' && (
            <div style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 11, color: scoreColor, fontWeight: 600, letterSpacing: '0.08em' }}>
              {correctCount} / {mcqTotal} MCQ CORRECT
            </div>
          )}
        </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '40px 56px 48px' }}>
          ...
          {phase === 'question' && currentQ && (
            <div>
              ...
              {currentQ.question_type === 'mcq' && currentQ.options && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {currentQ.options.map((opt, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => selectMcq(i)} style={{ textAlign: 'left', padding: '16px 20px', background: picked ? '#E8ECF7' : '#FFFFFF', border: `1.5px solid ${picked ? '#1F3A8A' : '#E4DFD3'}`, borderRadius: 6, cursor: 'pointer', fontFamily: 'Inter, system-ui', fontSize: 15, color: '#0B1220', lineHeight: 1.4, display: 'flex', alignItems: 'flex-start', gap: 14, transition: 'all .15s' }}>
              ...
              {/* Descriptive / Scenario text input */}
          ...
          {phase === 'results' && (
            <div>
              <div style={{ textAlign: 'center', marginBottom: 40, paddingBottom: 32, borderBottom: '1px solid #E4DFD3' }}>
                {showRing && <ScoreRing correct={correctCount} total={mcqTotal} />}
                <h2 style={{ fontFamily: '"Source Serif 4", serif', fontSize: 28, fontWeight: 500, color: '#0B1220', margin: '0 0 10px', letterSpacing: '-0.015em' }}>Quiz Submitted</h2>
                <div style={{ fontFamily: 'Inter, system-ui', fontSize: 14, color: '#5A6272', lineHeight: 1.5 }}>
                  <span style={{ fontWeight: 600, color: scoreColor }}>{correctCount} of {mcqTotal} MCQ correct</span>
                      ...
                      <div key={i} style={{ background: '#FFFFFF', border: `1px solid ${isCorrect ? '#C8DFC8' : 'rgba(196,48,43,0.22)'}`, borderRadius: 6, overflow: 'hidden' }}>
                        <div style={{ padding: '8px 16px', background: isCorrect ? 'rgba(31,122,58,0.07)' : 'rgba(196,48,43,0.06)', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${isCorrect ? '#C8DFC8' : 'rgba(196,48,43,0.2)'}` }}>
                          <div style={{ width: 20, height: 20, borderRadius: '50%', background: isCorrect ? 'rgba(31,122,58,0.15)' : 'rgba(196,48,43,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            {isCorrect ? <CheckIcon size={10} color="#1F7A3A" /> : <span style={{ color: '#C4302B', fontSize: 9, fontWeight: 700 }}>✕</span>}
                          </div>
                          <TypeBadge type="mcq" />
