"""
EHS Risk Assessment Chatbot — Groq + Llama 3.3 70B
Standalone script, completely independent of the EHS backend/RAG pipeline.
...
# Model options (Groq):
# "llama-3.3-70b-versatile"                      ← production, always works
# "meta-llama/llama-4-scout-17b-16e-instruct"    ← preview (enable in Groq console first)
# "meta-llama/llama-4-maverick-17b-128e-instruct" ← preview (enable in Groq console first)
# "openai/gpt-oss-120b"                           ← production, 120B reasoning model
...
# ── Page config ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="EHS Risk Assessment Assistant",
    page_icon="🦺",
    layout="wide",
)

st.title("EHS Risk Assessment Assistant")
st.caption("Powered by Llama 3.3 70B via Groq  |  Singapore WSH Regulations")
...
with st.sidebar:
    ...
    st.markdown(
        """
**How to use:**
- Describe a project and its major activities
- The assistant generates a full WSH risk assessment table
- Follow-up questions are supported in the same session

**Example prompts:**
- *"Generate a risk assessment for crane lifting operations on a Singapore construction site"*
- *"Risk assessment for electrical panel installation at height"*
- *"Confined space entry with chemical cleaning activities"*
- *"Hot work and welding operations in a shipyard"*
        """
    )
    st.markdown("---")
    if st.button("Clear Chat", use_container_width=True):
        ...
        st.rerun()
    st.markdown(f"**Messages in session:** {len(st.session_state.get('messages', []))}")
...
if "client" not in st.session_state:
    if not GROQ_API_KEY:
        st.error(
            ...
            "Add `GROQ_API_KEY=gsk_...` to your `.env` file or set it as an environment variable."
        )
        st.stop()
